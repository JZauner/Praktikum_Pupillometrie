...
execute:
  ...
  echo: false
params:
  author: "Versuchsleiter"
  proband: "Proband"
  auswerter: "Auswerter"
  filepath_tmp: "`r getwd()`"
  filename: "Table.png"
...
Table0 %>% 
  ...
  gtsave(file = params$filename2, path = params$filepath_tmp)

Tableinset <- png::readPNG(paste0(params$filepath_tmp, "/", params$filename2), native = TRUE)
...
Stat_Analysis %>% arrange(Parameter) %>% 
  ...
  tab_style(style = cell_text(size = 18), location = list(cells_body(rows = 1:3))) %>% 
   gtsave(file = params$filename, path = params$filepath_tmp, vwidth = 800)

```

\begin{figure}[!h]
  \centering
  \includegraphics[width=6.3in]{`r paste0(params$filepath_tmp, "/", params$filename)`}
\end{figure}
...
\hfill\small\emph{Versuchsleiter: `r params$author`, Proband: `r params$proband`, Auswerter: `r params$auswerter`}